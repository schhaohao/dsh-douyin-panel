/**
 * Shim geometry test (no dsh, no douyin): a fixed-width design page (min
 * content ≈ 576px) runs the REAL injected shim inside a 300px iframe; assert
 * overflow = scrollWidth − clientWidth ≈ 276 and the panel's one-step answer
 * (cw + overflow) ≈ 576, then that widening kills the overflow, then that
 * narrowing back restores it — the NO-AUTO-SHRINK asymmetry in practice.
 */
...
try {
  ...
  await page.setContent(`<iframe id="f" srcdoc="${escaped}" style="width:300px;height:400px;border:none"></iframe>`)
  const fits = []
  ...
  const iframeBox = await page.evaluate(() => document.getElementById('f').getBoundingClientRect().width)
  console.log('iframe width:', iframeBox, '| reports:', fits.length)
  ...
  await page.evaluate(() => { document.getElementById('f').style.width = '580px' })
  await page.waitForTimeout(1500)
  const fits2 = fits[fits.length - 1]
  check('at 580: overflow ≈ 0', fits2.overflowPx === 0, fits2.overflowPx)

  await page.evaluate(() => { document.getElementById('f').style.width = '300px' })
  await page.waitForTimeout(1500)
